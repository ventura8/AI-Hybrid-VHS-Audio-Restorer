...
- **Lint First**: Run `ruff check modules tests restore_audio_hybrid.py scripts/apply_patches.py` and `flake8 modules tests restore_audio_hybrid.py scripts/apply_patches.py`.
- **Static Analysis Gate**: Run `pylint --errors-only modules tests restore_audio_hybrid.py scripts/apply_patches.py` before `pytest`.
- **Auto-Fix**: Use `ruff format ...` and `ruff check --fix ...` for safe automatic fixes.
- **Manual Fix**: If lints remain after Ruff fixes, resolve them manually (without `noqa` or suppression pragmas).
...
- **Canonical Local Gate**: Run `./run_pipeline_locally.ps1`; it mirrors CI checks and regenerates the coverage badge.
...
- **Process Modes**:
    - `hybrid` (default/fallback): separation + vocal enhancement + background denoise + sync + final mix.
    - `denoise_only`: full-audio denoise + sync + final remux, with no separation or vocal enhancement.
- **Separation Strategy (`hybrid` only)**: The project uses a **Subtractive/Lossless Background** approach.
...
- **Implementation**: Sync logic is in `_align_stems` / `_align_stems_dtw`.
    ...
    - **Sequential Execution**: In `hybrid`, syncing Vocals and Background runs sequentially to maintain clean grouped log output.
    - **Mode Behavior**: In `denoise_only`, only one full-audio sync pass is executed.
...
- File scanning must exclude both cleaned output patterns to avoid reprocessing generated files.
