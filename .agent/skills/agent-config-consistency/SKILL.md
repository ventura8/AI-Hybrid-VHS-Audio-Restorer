...
1. Quality-gate command must be `./run_pipeline_locally.ps1`.
1. Dependency policy must stay Poetry-only and align with runtime/dev split.
1. Lint policy must keep max line length 140 and prohibit suppression bypasses.
1. CUDA runtime guidance must remain CUDA 13.2 aligned.
