...
def test_run_cathar_step_skips_when_exists(tmp_path):
    ...
    in_wav.write_bytes(b"input")
    out_wav.write_bytes(b"existing")
    out_wav.with_suffix(".json").write_text(
        json.dumps({"command": ["denoise"], "input_sha256": hashlib.sha256(b"input").hexdigest()}), encoding="utf-8"
    )
...
def test_find_quiet_window(tmp_path):
    ...
    with patch("modules.cathar._read_mono_samples", return_value=(fake_data[:, 0], 44100, 0)):
        ...
        tolerance = 1.0 / 44100
        assert 0.5 - tolerance <= start_s <= 1.0 + tolerance
...
def test_filter_cathar_vhs_pipeline(tmp_path):
    ...
    with (
        # The pipeline now refuses to start without the CLI. This test mocks every pass
        # below that guard, so it must mock the guard too rather than require the binary.
