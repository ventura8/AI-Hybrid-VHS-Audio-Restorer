...
def test_a_zero_factor_does_not_divide_the_native_bias_by_zero(tmp_path):
    """The native suppressor's bias scales with the factor; a factor of 0 keeps the configured bias."""
    seen = {}
    with (
        patch("modules.spectral_denoise.APL_SPECTRAL_ALPHA", 0.0),
        patch("modules.spectral_denoise.APL_SUPPRESS_NOISE_BIAS", 1.5),
        patch("modules.spectral_suppress.suppress_or_none", side_effect=lambda *_a, **kw: seen.update(kw) or None),
    ):
        assert spectral_denoise._native_suppress(tmp_path / "in.wav", tmp_path, 0.0, 4.0) is None
    assert seen["noise_bias"] == 1.5
