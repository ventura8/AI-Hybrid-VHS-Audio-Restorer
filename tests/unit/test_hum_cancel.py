...
def test_a_failure_while_planning_leaves_the_audio_usable(humming, tmp_path):
    """The long transform the plan reads can fail like the cancellation can; the stage skips rather than aborts."""
    source, _hum = humming
    with (
        patch.object(hum_cancel, "APL_ENABLE_HUM_CANCEL", True),
        patch.object(hum_cancel, "_plan", side_effect=MemoryError("no room")),
        patch("modules.hum_cancel.log_msg") as log,
    ):
        assert hum_cancel.apply_when_needed(source, tmp_path / "work") == source
    assert "after failure" in log.call_args[0][0]
    assert not (tmp_path / "work" / "hum_cancel").exists()
