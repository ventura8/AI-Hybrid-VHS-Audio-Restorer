...
def test_build_pre_denoise_surgical_filter():
    ...
    # The second harmonic belongs to pre-conditioning, which always runs before this stage.
    # Notching it here too cascaded Q=15 and Q=30 on the same tone.
    assert "bandreject=f=100.0:width_type=q:w=30" not in filter_str
...
def _pre_denoise_fallback_filter():
    ...
    return modules.filters.build_pre_denoise_surgical_filter(strategy)


def test_build_pre_denoise_surgical_filter_omits_preconditioned_bands():
    """Bands already handled during pre-conditioning must not be filtered a second time."""
    filter_str = _pre_denoise_fallback_filter()
    assert "highpass" not in filter_str
    assert "bandreject=f=60.0:width_type=q:w=30" not in filter_str
    assert "bandreject=f=15734.0:width_type=q:w=50" not in filter_str


def test_build_pre_denoise_surgical_filter_notches_the_third_harmonic_only():
    """120 Hz is the second harmonic of 60 Hz mains, already notched during
    pre-conditioning; the surgical pass starts at the third."""
    filter_str = _pre_denoise_fallback_filter()
    assert "bandreject=f=120.0:width_type=q:w=30" not in filter_str
    assert "bandreject=f=180.0:width_type=q:w=30" in filter_str
...
def test_cathar_find_quiet_window_bounds(tmp_path):
    ...
    with patch("modules.cathar._read_mono_samples", return_value=(np.zeros(100, dtype=np.float32), 44100, 0)):
...
def test_cathar_noiseprint_step_regenerates_corrupt_json(tmp_path):
    ...
    out_dir.mkdir()
    corrupt_json = out_dir / "noise_corrupt_test_single_6s.np.json"
