...
def test_the_factor_verdict_is_judged_on_the_speech_led_classes():
    """The corpus is speech-led, so the factor row is judged on speech and speech over a bed."""
    factor = check.COMPARISONS[0]
    assert factor[0].startswith("alpha 3.0 vs 1.8")
    assert factor[3] == check.SPEECH_LED_CLASSES == ("speech", "music")


def test_the_music_only_classes_are_the_factor_rows_blind_spot():
    """The music-only classes read a stronger factor as programme lost and are reported, not counted."""
    assert check.MUSIC_ONLY_CLASSES == ("musicled", "musiconly")
    assert set(check.SPEECH_LED_CLASSES + check.MUSIC_ONLY_CLASSES) == {"speech", *check.MUSIC_CLASSES}
    assert [spot[3] for spot in check.BLIND_SPOTS] == [check.MUSIC_ONLY_CLASSES]


def _trade_rows(removed, deviation, classes=("speech", "music", "musicled", "musiconly")):
    return [{"class": name, "noise_removed_db": removed, "programme_deviation_db": deviation} for name in classes]


def test_a_blind_spot_is_printed_as_known_and_does_not_count_against_agreement(capsys):
    """A reading the fixtures are known to miss appears under the table marked known, and the agreement count ignores it."""
    results = {
        "no_tonal_gate": {"trade": _trade_rows(12.0, 0.8)},
        "alpha_1_8": {"trade": _trade_rows(10.0, 0.5)},
        "current": {"trade": _trade_rows(12.0, 0.7)},
        "apl_probe_0s75": {"trade": _trade_rows(9.0, 0.6)},
        "deepfilternet": {"trade": _trade_rows(10.0, 1.0)},
    }
