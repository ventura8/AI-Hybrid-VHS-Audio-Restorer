...
if modules.utils.sf is None:
    modules.utils.sf = MagicMock()
...
def test_save_atomic_success(mock_write, mock_valid, tmp_path):
    ...
    # Call the function
    modules.utils._save_audio_atomic(str(output_file), b"test data", 44100)
...
def test_draw_progress_bar_min_width(mock_time, mock_cols, mock_draw):
    ...
    called_width = mock_draw.call_args[0][0]
...
@patch("modules.utils.sf.write")
def test_save_audio_atomic_replaces_existing_file(mock_write, mock_valid, mock_log, tmp_path):
...
@patch("modules.utils.log_msg")
@patch("modules.utils.sf.write")
def test_save_audio_atomic_exception_cleanup(mock_write, mock_log, tmp_path):
    ...
    mock_write.side_effect = RuntimeError("disk error")
