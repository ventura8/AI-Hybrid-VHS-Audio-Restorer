...
def test_collect_eligible_tapes_handles_invalid_directory(tmp_path):
    """Gracefully return empty list when input_dir is missing or a file."""
    nonexistent = tmp_path / "nonexistent_dir"
    assert run_multi_pass_tuning._collect_eligible_tapes(nonexistent, 60.0, 120.0) == []

    file_path = tmp_path / "file.txt"
    file_path.touch()
    assert run_multi_pass_tuning._collect_eligible_tapes(file_path, 60.0, 120.0) == []
