...
def test_auto_mode_execute(tmp_path):
    """AutoMode delegates to the scanner-driven pipeline and ignores a preset strategy."""
    mode = AutoMode()
    ...
    with patch("modules.processing._process_auto_mode") as mock_auto:
        mode.execute(work_dir, orig, vid, out, 20.0, strategy={"denoise_model": "ignored.pth"})

    mock_auto.assert_called_once_with(work_dir, orig, vid, out, 20.0)
...
def test_cathar_mode_execute(tmp_path):
    ...
    with (
        patch("modules.processing._resolve_preconditioned_audio", return_value=(orig, {})) as mock_pre,
        patch("modules.processing._process_single_track_pipeline") as mock_pipeline,
        patch("modules.cathar.filter_cathar_vhs_pipeline", return_value=orig) as mock_cathar,
