...
@functools.cache
def _ffmpeg_has_filter(binary, filter_name):
    """Probe one resolved FFmpeg binary for a named filter."""
    cmd = [binary, "-filters"]
    try:
...
def ffmpeg_has_filter(filter_name):
    """Check for a filter, caching independently for each FFmpeg executable."""
    return _ffmpeg_has_filter(FFMPEG_BIN, filter_name)
