...
def _long_spectrum(mono_signal, sample_rate):
    """One long transform of the opening stretch, bins of 0.04 Hz, or (None, None) when the stretch is too short."""
    segment = mono_signal[:REFINE_SAMPLES]
    if len(segment) < MIN_FRAMES * ANALYSIS_HOP:
        return None, None
...
def refine_f0(mono_signal, sample_rate, f0, harmonics):
    ...
    """
    spectrum, freqs = _long_spectrum(mono_signal, sample_rate)
    if spectrum is None or not harmonics:
        return f0
...
def apply_when_needed(source_wav, audio_dir, strategy=None):
    ...
    try:
        # Planning reads a long transform of the opening stretch, so it can fail the same
        # way the cancellation can, and is guarded the same way.
