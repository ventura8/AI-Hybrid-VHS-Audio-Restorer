...
# The enhancement input/output glue lives with the chunking it drives; the private alias
# keeps the previous call sites and their tests working.
_handle_enhance_output = _chunking.handle_enhance_output
...
def _cpu_variant_of(cmd_enhance):
    """Returns the command retargeted at the CPU, or None when it is already CPU-bound."""
    try:
        device_at = cmd_enhance.index("--device") + 1
    except ValueError:
        return None
    if device_at >= len(cmd_enhance) or str(cmd_enhance[device_at]).lower() == "cpu":
        return None
    cpu_cmd = list(cmd_enhance)
    cpu_cmd[device_at] = "cpu"
    return cpu_cmd


def _run_enhance_retry(cmd_enhance, total_duration, env=None):
    """Executes enhancement with GPU retries, then a CPU safety net.

    Chunking is the primary defence against running out of VRAM; this is the net beneath it.
    A card smaller than any chunk size, a driver that fragments badly, or another process
    taking the GPU mid-run would otherwise fail the whole restoration. The CPU pass is slow
    but finishes, which is preferable to losing the job.
    """
    max_retries = 2
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                time.sleep(2)
                continue
            cpu_cmd = _cpu_variant_of(cmd_enhance)
            if cpu_cmd is None:
                raise e
            log_msg("    [Warning] GPU enhancement failed; falling back to CPU (slower).", is_error=True)
            _clear_cuda_retry_state()
            run_command_with_progress(cpu_cmd, env=env, description="Enhancing Vocals (CPU)", total_duration=total_duration)
            return
...
def _enhance_vocals_step(vocals_wav, enhanced_vocals_dir, work_dir, total_duration=None, enhance_nfe=None, enhance_tau=None):
    ...
    chunk_paths = _chunking.prepare_enhance_input(vocals_wav, enhance_input_dir, total_duration)
    ...
    result = _chunking.collect_enhance_result(enhanced_vocals_tmp_dir, vocals_wav, chunk_paths)
...
def _polish_full_audio_step(denoised_wav, polish_dir, total_duration=None, strategy=None, apply_air=False):
    ...
    output_wav = polish_dir / f"polished_{fingerprint}_{denoised_wav.name}"
    if is_valid_audio(output_wav):
        return output_wav
...
def _pre_denoise_surgical_step(precond_wav, audio_dir, total_duration=None, strategy=None):
    ...
    output_wav = audio_dir / f"surgical_{fingerprint}_{precond_wav.name}"
    if is_valid_audio(output_wav):
        return output_wav
...
def _post_denoise_cleanup_step(denoised_wav, audio_dir, total_duration=None, strategy=None):
    ...
    if not cleanup_filter:
        return denoised_wav
    fingerprint = hashlib.sha256(cleanup_filter.encode("utf-8")).hexdigest()[:12]
    output_wav = audio_dir / f"cleaned_{fingerprint}_{denoised_wav.name}"
    if is_valid_audio(output_wav):
        return output_wav
