...
defect and measured free where the defect is absent; the 1.24 dB of extra
removal over v1.2.1 comes from a noise probe lengthened to 4 s on the evidence
of this corpus, at the same median deviation.
...
`auto_pure_linear` removes more noise on 112 of 136 clips and disturbs
the programme less on 86 of 136. On 72 clips it wins both at once, against 10
where `cathar` wins both.

| Mains hum (median, 33 readable hum tapes) | `cathar` | `auto_pure_linear` |
| :--- | ---: | ---: |
| Hum removed (dB of harmonic excess, higher better) | -0.74 | **2.43** |
| Hum lines taken down (dB, gain-matched) | 0.75 | **3.13** |
| Low band moved (dB, 50-400 Hz in loud frames) | 0.55 | 0.59 |
...
| Programme deviation (dB) | 0.32 | **0.25** |

`auto_pure_linear` removes more noise on 60 of 69 clips and deviates less on
42; it wins both on 37 against 4.
...
-0.05 dB. It removes more noise on 52 of 67 clips there and deviates less on
44, winning both on 35 against 6. The 4 s probe spent part of the mode's NTSC
...
| `auto_pure_linear` | **2.51** | **6.33** | **9.99** | **18.14** | **37.33** |
...
- **A hum canceller** (`modules/hum_cancel.py`), ahead of the noise probe. Each
  ...
  and a 6 dB cap over its running level. Run alone on the 48 hum tapes it removes
  a median 3.01 dB of harmonic excess at 0.155 dB of low-band movement, where
  `cathar dehum` at the right frequency removes 2.07 at 0.25; in the chain, hum
  removed goes from 0.19 dB to 2.49 on all 48 at the probe the stage was
  adopted at, better on 34 of 48 and worse by more than a decibel on none;
  section 4 has the final build's table on the readable tapes.
- **Event-gated plosive control** (`modules/plosive_tamer.py`). A blast under
  ...
  costs 0.5-0.9 dB on undamaged classes this costs 0.00-0.01. On real tape it
  is free.
- **A 4 s noise probe**, up from 2.5. v1.2.1 chose 2.5 s on 25 clips where the
  ...
  `cathar` on the same 72 clips and loses both on 10 rather than 11. The cost
  is in the deviation tail -- the upper quartile moves 0.76 to 0.91 dB, 17
  clips move up by more than 0.2 dB and 8 move down -- and on NTSC, whose
  median deviation goes 0.33 to 0.45 against `cathar`'s 0.57. 6 s removes
  10.64 at 0.35 and loses both on 13; 8 s read 12.64/0.27 on 50 clips and was
  not taken further. On a long tape the quietest 4 s is far likelier to be
  pure noise than it is inside a 15 s corpus clip, so the corpus reads this
  setting's risk high rather than low. An adaptive length -- extend the
  quietest window while its spectrum, level or spread stays close to the
  2.5 s window's -- was calibrated against that per-clip cost and predicts
  none of it (correlations of 0.01 to 0.06), so the length is fixed.
...
- **Candidates measured and held back**, with their numbers in the
  configuration comments so the next round does not repeat them:
  - A per-bin MMSE log-spectral suppressor in the subtraction slot
    ...
    subtraction's 10.02 at 0.22 (3.06/0.22 against 7.96/0.28 on the tonal 45).
    A per-bin estimator keeps the low-level programme the quiet frames hold,
    which the trade metric reads as noise left behind -- the DeepFilterNet
    lesson a second time.
  - The Mel-Roformer denoiser as the neural stage: 9.39/0.22 against
    UVR-DeNoise's 10.02/0.23.
  - Resemble-Enhance's denoiser as the neural stage (`--denoise_only`, a
    masking model; the package's enhancer is generative and was never a
    candidate): 10.78/0.58 against UVR-DeNoise's 10.02/0.23 on 50 captures.
    More removal, the programme moved two and a half times as far, and 12
    captures won outright against `cathar` where UVR-DeNoise wins 28..
...
| `auto_pure_linear` v1.3.0 | **2.43 dB** | **5.04 dB** | **3.13 dB** | 0.59 dB |
...
`auto_pure_linear` removes more excess than `cathar` on 27 of the 33 tapes
and takes the lines themselves further down on 25; on 18 it ends past the
...
read beside it (on 3 of the 33 it rises after `auto_pure_linear`, on 12
...
50-400 Hz). Neither mode has a stage for it beyond the shared pre-conditioning
highpass, and on the corpus that reads a median 20.49 dB drop in the
...
| `auto_pure_linear` v1.3.0 | 2.40 | 2.96 | 1.81 | +0.32 |

`auto_pure_linear` reads better than `cathar` on 137 of 174 clips
...
- **The clips it wins outright.** Ten clips, against eleven in v1.2.1: eight
  of that release's eleven, of which three moved to a split, and two
  newcomers. Six of the ten are NTSC and five sit in the most tonal third of
  the corpus (flatness under 0.036), and on every one the pattern is the
...
| rock-with-barney-1991-1993-vhs-release... | NTSC | 0.82/1.02 | 0.28/1.18 |
| the-agatha-christie-hour-in-a-glass-deadly... | PAL | 6.17/0.23 | 4.42/0.71 |
...
| vid-20230624-142023 | PAL | 44.39/0.07 | 39.29/0.14 |
...
- **Sustained tonal programme -- music albums, a held score under
  ...
  corpus, where half of its ten outright wins sit, at the cost of removing
