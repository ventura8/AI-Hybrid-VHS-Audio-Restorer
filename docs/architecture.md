...
Applied by `auto_pure`, `auto_pure_linear`, and `multipass_auto` before any AI
stage. Every filter is conditional: the chain collapses to `anull` when no
defect is detected.
...
Like `auto_pure` and `multipass_auto`, this mode reuses the shared Pass 2 analog
