...
```powershell
.\.venv\Scripts\python.exe -m poetry run ruff check modules tests restore_audio_hybrid.py scripts/apply_patches.py
.\.venv\Scripts\python.exe -m poetry run flake8 modules tests restore_audio_hybrid.py scripts/apply_patches.py
.\.venv\Scripts\python.exe -m poetry run pytest -o addopts= --cov=restore_audio_hybrid --cov=modules --cov-branch \
	--cov-report=xml --cov-report=json --cov-report=term --cov-fail-under=90 tests/
.\.venv\Scripts\python.exe -m poetry run python tests/tooling/quality_gate.py coverage.json --threshold 90.0
```
