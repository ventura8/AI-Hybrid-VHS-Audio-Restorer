...
1. **A hum canceller of the mode's own**: every mains harmonic that stands out
   ...
   5.87) at adoption and, once three gates keep a chord's partials out of the
   series, 1.19 on the final build's 33 readable tapes, with the lines
   themselves down 3.04 dB against `cathar`'s 0.75.
...
1. **Candidates measured and held back**: a per-bin MMSE log-spectral
   ...
   Mel-Roformer denoiser (less removal than UVR-DeNoise), a shorter probe and
   the neural stage left out on tonal material (neither cuts the tonal losses),
   Resemble-Enhance's
...
1. **The default mode**: on the full corpus the mode leads `cathar` on every
   row measured on real tape -- broadband noise (10.10/0.31 against 6.02/0.44,
   winning both on 70 clips against 8), mains hum (1.19 dB against -0.74 on
...
- **Rumble**: the mode relies on the shared pre-conditioning highpass;
  ...
  been measured.
- **Hum on tonal programme**: the canceller now refuses a series whose lines
  place the fundamental off the mains window, keeps only lines at exact
  multiples of the fundamental, and refuses a series with nothing at the
  fundamental, after the music-only and `ep` fixture classes caught it taking
  chord partials and a voice's harmonics for hum; the hum instruments cannot see
  programme removed at mains multiples, so the fixture's reference is the only
  guard there.
