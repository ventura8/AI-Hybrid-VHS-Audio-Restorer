...
EXTS = {str(extension).lower() for extension in CONFIG["extensions"]}
...
def _extract_pair_clips(clips_dir, trial, labels, start):
    ...
    for label, (_, source_path) in zip(("A", "B"), labels, strict=True):
        if not _extract_clip(source_path, clips_dir / f"{trial}_{label}.wav", start):
            for cleanup_label in ("A", "B"):
                (clips_dir / f"{trial}_{cleanup_label}.wav").unlink(missing_ok=True)
            return False
    return True
...
def _write_blind_sheets(output_dir, review_rows, key_rows, key_path=None):
    """Writes the listener-facing scoresheet, and the randomization key outside their reach.

    The key maps each trial back to its A/B modes. Written beside blind_review.csv and
    blind_clips/, any listener opening the output directory can read it, which removes the
    blinding this script exists to provide. It goes to a sibling path by default.
    """
    _write_csv(
    ...
    key_target = Path(key_path) if key_path else output_dir.parent / f"{output_dir.name}_blind_key.csv"
    key_target.parent.mkdir(parents=True, exist_ok=True)
    _write_csv(key_target, ["trial", "source", "clip_start_seconds", "A_mode", "B_mode"], key_rows)
    print(f"Randomization key (keep away from listeners): {key_target}")


def _build_blind_materials(results, output_dir, key_path=None):
    ...
    _write_blind_sheets(output_dir, review_rows, key_rows, key_path=key_path)
...
def main():
    ...
    if not args.skip_clips:
        _build_blind_materials(results, output_dir, key_path=args.key_path)
