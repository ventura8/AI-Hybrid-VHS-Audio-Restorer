...
def _is_valid_capture(path, valid_exts):
    """Return whether a path is a supported capture file."""
    return path.is_file() and path.suffix.lower() in valid_exts


def _scan_directory_captures(directory):
    """Return sorted captures using normalized configured extensions."""
    valid_exts = {extension.lower() for extension in config_mod.EXTS}
    return sorted(path for path in directory.iterdir() if _is_valid_capture(path, valid_exts))
...
def _average_metrics(deltas_list: List[Dict[str, float]]) -> Dict[str, float]:
    """Computes average values for the metrics present in benchmark deltas."""
    if not deltas_list:
        return {}
    keys = {key for delta in deltas_list for key in delta}
    return {key: round(float(np.mean([delta[key] for delta in deltas_list if key in delta])), 2) for key in keys}


def _aggregate_by_region(clips_data: List[Dict[str, Any]], mode: str) -> Dict[str, Dict[str, float]]:
    """Aggregates restored-mode deltas by region."""
    return _aggregate_by_value(clips_data, mode, "region", ("europe", "america"))


def _aggregate_by_genre(clips_data: List[Dict[str, Any]], mode: str) -> Dict[str, Dict[str, float]]:
    """Aggregates restored-mode deltas by every observed genre."""
    genres = sorted({str(clip.get("genre", "general")) for clip in clips_data})
    return _aggregate_by_value(clips_data, mode, "genre", genres)


def _aggregate_by_value(clips_data, mode, key, values):
    """Groups mode deltas by one metadata key."""
    return {
        value: _average_metrics(
            [clip["restored"][mode]["deltas"] for clip in clips_data if clip.get(key) == value and mode in clip.get("restored", {})]
        )
        for value in values
    }
