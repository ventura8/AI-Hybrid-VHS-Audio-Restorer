...
def _extract_audio_sample(tape_path, temp_wav, start_sec, duration_sec):
    ...
    try:
        ...
        # 60s budget killed FFmpeg on longer samples or slow storage, and the batch loop then
        # reported "Audio extraction or analysis failed" without naming the timeout.
        timeout_sec = max(60, int(duration_sec) * 2)
        res = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False, timeout=timeout_sec)
...
def run_batch_scan(input_target, output_json, start_sec=30, duration_sec=90):
    ...
    for tape_path in candidates:
        ...
        if res is None:
            print(f"  [Warning] Audio extraction or analysis failed: {tape_path}", file=sys.stderr)
            continue
        results.append(res)
