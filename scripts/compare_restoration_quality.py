...
VIDEO_EXTENSIONS = {".mp4", ".mpg", ".mkv", ".avi", ".mov", ".m4v"}
...
def _collect_comparison_metrics(files, work_dir, crt_hz):
    ...
    for label, file_path in files:
        ...
        is_temporary = file_path.suffix.lower() in VIDEO_EXTENSIONS
        ...
        try:
            ...
            # One bad candidate must not take the comparison down with it. The point of this
            # tool is to rank several candidates against each other; aborting on the first
            # unreadable one discards the measurements already taken for the rest.
            print(f"Skipping {label}: could not analyse '{file_path}' ({exc})")
        finally:
            if is_temporary:
                eval_wav.unlink(missing_ok=True)
    return results
...
def main():
    ...
    results = compare_files(args.original, restored_pairs, work_dir=args.work_dir, crt_hz=args.crt_hz)
    # An empty table used to exit 0, so automated validation could record a successful quality
    # comparison that measured nothing. A comparison also needs the original to compare against.
    labels = {row["label"] for row in results}
    if "ORIGINAL" not in labels:
        raise SystemExit("Comparison failed: the original could not be analysed.")
    if len(labels) < 2:
        raise SystemExit("Comparison failed: no restored candidate could be analysed.")
