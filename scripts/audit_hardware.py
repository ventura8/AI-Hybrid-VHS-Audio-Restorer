...
def build_report():
    ...
    return {
        ...
        # The same predicate require_nvidia_cuda() enforces. cpu_only_fallback alone is False on
        # Apple MPS too, so the audit would report CUDA readiness on a machine with no NVIDIA GPU
        # -- and scripts/remote_validate.sh treats this field as proof that CUDA is in use.
        "pytorch_cuda_ready": settings["is_nvidia"] and not settings["cpu_only_fallback"],
