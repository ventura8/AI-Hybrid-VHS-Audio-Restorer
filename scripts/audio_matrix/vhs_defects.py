...
SUPPORTED_DEFECTS = {"hiss", "hum", "whistle", "rumble", "clicks", "dropout", "clip", "azimuth", "balance", "drift"}
...
def apply_vhs_defects(samples, sample_rate, defects):
    ...
    unknown = set(defects).difference(SUPPORTED_DEFECTS)
    if unknown:
        raise ValueError(f"Unknown VHS defect(s): {', '.join(sorted(unknown))}")
