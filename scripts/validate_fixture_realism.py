...
- subtraction factor 3.0 beats 1.8         (50 clips: removal +3.35 dB for +0.07 deviation)
...
The factor verdict is judged on the speech-led classes -- speech, and speech over a music
bed -- because that is what the corpus is made of, and the music-only classes are a
recorded blind spot for it: a fixture with no quiet frames that are noise reads a stronger
factor as programme lost, -1.34 dB of removal for +2.36 of deviation over ten fixtures,
where real tape's most tonal tenth reads +2.77 dB for +0.00 and its most tonal third
+2.52 for +0.23. That reading is printed under the table and excused from the exit
status, the way the two classes the scanner cannot see are.
...
# Speech, and speech over a music bed: the programme the corpus is made of.
SPEECH_LED_CLASSES = ("speech", "music")
MUSIC_ONLY_CLASSES = ("musicled", "musiconly")

# (label, better variant, worse variant, classes to judge on, rule, what real tape found)
COMPARISONS = (
    (
        "alpha 3.0 vs 1.8 (speech-led)",
        "no_tonal_gate",
        "alpha_1_8",
        SPEECH_LED_CLASSES,
        REMOVAL_DOMINATES,
        "+3.35 dB removal for +0.07 deviation",
    ),
...
# Comparisons a fixture class is known not to reproduce: reported under the table with the
# real-tape reading they miss, and excused from the exit status.
# (label, better variant, worse variant, classes, why)
BLIND_SPOTS = (
    (
        "alpha 3.0 vs 1.8 (music-only)",
        "no_tonal_gate",
        "alpha_1_8",
        MUSIC_ONLY_CLASSES,
        "no quiet frames that are noise, so the factor's cost reads as programme lost; real tape's most tonal tenth: +2.77 dB for +0.00",
    ),
)
DEFAULT_VARIANTS = (
    "speech_m12",
...
def _trade_medians(results, better, worse, classes):
    """The two configurations' median removal and deviation over the classes: (rb, rw, db, dw)."""
    return (
        _median(results[better]["trade"], "noise_removed_db", classes),
        _median(results[worse]["trade"], "noise_removed_db", classes),
        _median(results[better]["trade"], "programme_deviation_db", classes),
        _median(results[worse]["trade"], "programme_deviation_db", classes),
    )
...
def _report_trade(results):
    ...
    for label, better, worse, classes, rule, verdict in COMPARISONS:
        rb, rw, db, dw = _trade_medians(results, better, worse, classes)
    ...
    _report_blind_spots(results)
    print(f"\nagreement with real tape: {agree}/{len(COMPARISONS)}")
    return agree


def _report_blind_spots(results):
    """The comparisons a fixture class is known not to reproduce, printed under the table and not counted."""
    for label, better, worse, classes, why in BLIND_SPOTS:
        rb, rw, db, dw = _trade_medians(results, better, worse, classes)
        if None in (rb, rw, db, dw):
            print(f"{label:<32}{'unmeasured':>18}")
            continue
        print(f"{label:<32}{rb:>9.2f}{rw:>9.2f}{db:>11.2f}{dw:>11.2f}   known ({why})")
