...
def _is_eligible_tape(f, min_duration_min, max_duration_min):
    """Return tape tuple if file matches video extension and duration range."""
    if f.suffix.lower() not in _VIDEO_EXTS:
        return None
    dur = _probe_duration(f)
    if dur is not None and min_duration_min <= dur <= max_duration_min:
        return (f, round(dur, 1))
    return None
...
def _collect_eligible_tapes(input_dir, min_duration_min, max_duration_min):
    ...
    for f in sorted(path.iterdir()):
        tape = _is_eligible_tape(f, min_duration_min, max_duration_min)
        if tape is not None:
            tapes.append(tape)
    return tapes
...
def _run_single_pass(tape_path, mode, clip_dur, start_sec, output_dir, gpu_name, work_dir, crt_hz=15625.0):
    ...
    extension_tag = tape_path.suffix.lstrip(".").lower() or "video"
    sample = output_dir / f"{tape_path.stem}_{extension_tag}_sample_{clip_dur}s.mp4"
...
def _compute_delta(original, restored):
    ...
    crt_attn = original.get("crt_whistle_ratio", 1) / max(restored.get("crt_whistle_ratio", 1), 0.05)
...
def _rec_filter_checks(rec, nf_mean, crt_mean, rumble_mean, crt_hz):
...
def _rec_for_mode(stats, crt_hz):
    ...
    _rec_filter_checks(rec, nf_mean, crt_mean, rumble_mean, crt_hz)
    return rec


def _recommend_parameters(pass_aggregates, crt_hz):
    """Derives config parameter recommendations from aggregated multi-pass data."""
    return {mode: _rec_for_mode(stats, crt_hz) for mode, stats in pass_aggregates.items()}
...
def main():
    ...
    # (`del gpu_name`), so a --device flag could only ever change this line of output while
    # the restoration ran on whatever modules.hardware selected -- a switch that appeared to
    ...
    recommendations = _recommend_parameters(aggregated, args.crt_hz)
