...
def _has_requested_modes(result: Dict[str, Any], modes: List[str]) -> bool:
    """Return whether a checkpoint result contains every requested restoration mode."""
    restored = result.get("restored", {})
    return all(mode in restored for mode in modes)


def _ready_clip_path(corpus_dir: Path, item: Dict[str, Any], completed_ids: set[str], skipped: List[Dict[str, str]]) -> Optional[Path]:
    """Return an eligible fixture path, recording why a clip is being passed over.

    An already-completed clip is not recorded: it contributes results from the checkpoint and
    is not missing from the run. A catalog entry with no file, or one whose file is absent, is
    recorded -- those used to disappear, leaving the report's averages covering an unstated
    ...
    if not relative_file:
        skipped.append({"clip": identifier, "reason": "catalog entry has no file"})
        return None
    clip_path = corpus_dir / relative_file
    if not clip_path.exists():
        skipped.append({"clip": identifier, "reason": f"file not found: {relative_file}"})
        return None
    return clip_path
...
def _record_result(results, result, identifier, result_indexes, checkpoint_file, index, total) -> None:
    """Replace a checkpoint result and persist each checkpoint boundary."""
    if result is None:
        return
    existing_index = result_indexes.get(identifier)
    if existing_index is None:
        result_indexes[identifier] = len(results)
...
def _benchmark_catalog(args, catalog, results, eval_dir, checkpoint_file) -> List[Dict[str, str]]:
    """Benchmark each available unprocessed catalog entry, returning what it had to skip."""
    result_indexes = {result["identifier"]: index for index, result in enumerate(results)}
    completed_ids = {identifier for identifier, index in result_indexes.items() if _has_requested_modes(results[index], args.modes)}
    skipped: List[Dict[str, str]] = []
    total = len(catalog)
    for index, item in enumerate(catalog, 1):
        identifier = item["identifier"]
        clip_path = _ready_clip_path(args.corpus_dir, item, completed_ids, skipped)
        if clip_path is None:
            continue
        sys.stdout.write(f"[{index}/{total}] Benchmarking {identifier}...\n")
        result = _run_clip_benchmark(args, item, clip_path, eval_dir)
        if result is None:
            skipped.append({"clip": identifier, "reason": "benchmark run produced no result"})
        _record_result(results, result, identifier, result_indexes, checkpoint_file, index, total)
        if result is not None and _has_requested_modes(result, args.modes):
...
def main():
    ...
    skipped = _benchmark_catalog(args, catalog, results, eval_dir, checkpoint_file)
    _save_json(checkpoint_file, results)
    ...
    if skipped:
        sys.stderr.write(f"\n[Coverage] {len(results)} of {len(catalog)} catalog clips evaluated, {len(skipped)} skipped:\n")
        for entry in skipped:
            sys.stderr.write(f"  - {entry['clip']}: {entry['reason']}\n")
