...
description: >-
  Verify and resolve an entire CodeRabbit review wave without premature
  handoff. Use when a user supplies CodeRabbit findings, asks to finish a
  review wave, or requires post-review hardware validation.
---
...
1. Inspect the referenced code and its callers/tests. Do not blindly follow a
   suggested patch.
1. Apply minimal, behavior-preserving changes for valid findings. Preserve
   existing user changes and do not add suppression directives.
1. Add or update regression tests for behavior changes and edge cases.
1. Run focused formatting, linting, and tests for all touched files. Run the
   canonical local quality gate when its duration and environment permit.
1. If audio, DSP, sync, model, installer, or hardware code changed, run the
...
1. Inspect the resulting report: confirm CUDA/device readiness, execution count,
